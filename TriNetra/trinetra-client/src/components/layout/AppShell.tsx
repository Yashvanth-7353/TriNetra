...
export default function AppShell() {
  ...
  const SidebarContent = (
    <>
      <Link to="/" className="p-4 flex items-center gap-3 border-b border-primary-800 hover:bg-primary-800 transition-colors cursor-pointer" onClick={() => setIsMobileMenuOpen(false)}>
  ...
  return (
    <div className="flex h-screen w-full bg-white text-slate-900">
      {/* Desktop Sidebar */}
      <aside className="w-64 bg-primary-900 text-white flex-col transition-all duration-300 hidden md:flex shrink-0">
        {SidebarContent}
      </aside>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[100] flex">
          ...
          <div className="relative w-72 bg-primary-900 text-white flex flex-col h-full shadow-2xl overflow-hidden pt-12">
            <button 
              onClick={() => setIsMobileMenuOpen(false)}
