# Sahayak-AI / Netra: Intelligent Conversational AI & Crime Analytics Platform
...
Deployed fully within the mandatory **Zoho Catalyst** cloud ecosystem using a **React frontend** and a **Node.js backend orchestrator**.
...
The entire application runs inside the Zoho Catalyst environment, leveraging the unified `catalyst-node-sdk` to minimize infrastructure friction.
...
| **Backend Orchestrator** | Node.js (Express / NestJS) | **AppSail (Managed Runtime)** | Houses the intent classifier, conversational state manager, transaction tracker, and response assembler. |
...
| **Perceptual Processing** | Voice and Translation | **Zia Services** | Dual-language pipeline (Kannada $\leftrightarrow$ English translation, Speech-to-Text, and Text-to-Speech synthesis). |
...
3. **Intent Classification:** The Node.js application inside **AppSail** scans the text and classifies it into one of four distinct technical execution categories:
...
To maintain 100% Catalyst compliance without external Graph databases, the platform implements a relationship crawler directly inside Node.js over indexed relational lookups:
...
* The Node.js backend converts these multi-hop records into a Node-Edge JSON structure.
...
[ Phase 1: Hackathon MVP ] ──► [ Phase 2: System Expansion ] ──► [ Phase 3: Hardened Release ]
   ...
   • AppSail Node.js Router        • Full Tabular Zia AutoML          • Real-Time Data Sync (CDC)
...
* Node.js v18 or above installed locally.
...
*Select your active project space, link the Data Store configuration, and map AppSail to the Node.js project directory.*
